...
def _deserialize_output_targets(
...
def _attention_block_for_layer(model, layer_index: int):
    ...
    if transformer is not None and hasattr(transformer, "h"):
        if not 0 <= int(layer_index) < len(transformer.h):
            return None
        return transformer.h[layer_index].attn
...
def _cache_layer_key_value(cache, layer_index: int):
    ...
    try:
        layer_entry = cache[layer_index]
    except (TypeError, IndexError, KeyError):
        return None
    if layer_entry is None or len(layer_entry) < 2:
        return None
    layer_keys, layer_values = layer_entry[:2]
    if layer_keys is None or layer_values is None:
        return None
    if getattr(layer_keys, "ndim", 0) >= 4:
        layer_keys = layer_keys[0]
    if getattr(layer_values, "ndim", 0) >= 4:
...
def collect_teacher_forced_boundary_collection(
    ...
    *,
    model=None,
    tokenizer=None,
    probe_layers: tuple[int, ...] = PROBE_LAYERS,
    probe_heads: tuple[int, ...] = PROBE_HEADS,
...
def _collect_boundary_collection_with_model(
    *,
    ...
    if len(token_ids) < prefix_token_count:
        raise ValueError(
            "Materialized token ids are shorter than the prefix token count: "
            f"{len(token_ids)} < {prefix_token_count}."
        )
                ...
                progress_callback(
                    {
                        ...
                        "capture_token_count": len(capture_indices),
                    }
                )
