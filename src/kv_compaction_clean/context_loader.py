...
CLEAN_REPO_ROOT = Path(__file__).resolve().parents[2]
LOCAL_DATASET_PATHS = {
    "local_placeholder": CLEAN_REPO_ROOT / "data/smoke_test/local_placeholder.json",
}
...
def _resolve_dataset_path(dataset: str) -> Path:
    ...
    if not candidate.is_absolute():
        candidate = CLEAN_REPO_ROOT / candidate
    return candidate
...
def load_context_sample(config: SmokeTestConfig, sample_id: str | None = None) -> LoadedContextSample:
    ...
    compaction_chunk_ids = [
        chunk.chunk_id for chunk in chunks if chunk.start_token < prefix_token_count
    ]
    ...
    return LoadedContextSample(
        ...
        prompt_family=str(raw_sample.get("prompt_family", "warehouse_migration")),
        turns=turns,
        chunks=chunks,
        logical_context_tokens=total_tokens,
        physical_context_tokens=total_tokens,
